...
//Changed to adapt Dark Souls III

namespace ModUtils
{
	static HWND muWindow = NULL;
	static std::string muGameName = "DARK SOULS™ III";
	static std::string muExpectedWindowName = "DARK SOULS III";
	...
	/**Write {@code 14+extraClearance} bytes from {@param address} to a {@code jmp qword ptr} instruction (6 bytes)
	 * and the location of {@param destination}. As it overwrites original codes, may copy them to the dest
	 * @param address the bytes written FROM
	 * @param destination Codes jmp to. May have Address' code copy.
	 * @param extraClearance more bytes to write
	 */
